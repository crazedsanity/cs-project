...
class taskClass {
	 ...
	 * The constructor. Duh.
	 */
	 function taskClass(&$db,$projectId=NULL,$taskId=NULL) {
	...
	//================================================================================================
	/**
	 * Extensible method to grab a list of task's based upon the given criteria.
	 * 
	 ...
	 * @return <array>		PASS: list of task's, keyed off their id.
	 */
	function get_tasks(array $critArr, $primaryOrder=NULL, array $contactCrit=NULL) {
		...
		if(!preg_match('/task_id/', $orderBy) || (!strlen($orderBy))) {
		...
		if($dberror || $numrows < 1) {
			...
			//good data.
			$retval = $this->db->farray_fieldnames("task_id",NULL,0);
	...
	//================================================================================================
	/**
	 * A wrapper for get_tasks() to retrieve a single task's info.
	 * 
	 * @param $taskId		<int> id of the task to retrieve.
	 * 
	 * @return <SPECIAL: returns the requested record using get_tasks()>
	 */
	function get_task($taskId=NULL) {
		if(!is_numeric($taskId)) {
			$retval = 0;
		}
		else {
			
			$retval = $this->get_tasks(array("task_id"=>$taskId));
			$retval = $retval[$taskId];
		}
	...
	//================================================================================================
	/**
	 * Updates a single task record with the specified data.
	 * 
	 * @param $taskId		<int>
	 ...
	 */
	function update_task($taskId, $updatesArr) {
		...
		$updatesArr = $this->check_for_auto_updates($taskId, $updatesArr);
		
		if(is_numeric($updatesArr['add_elapsed'])) {
			//they're adding time to elapsed.
			$this->add_elapsed($taskId, $updatesArr['add_elapsed'], $this->logsObj->defaultUid);
		}
		
		//get the old data for logging & such.
		$oldData = $this->get_task($taskId);
		...
		$sql = "UPDATE task_table SET $updateStr WHERE task_id=$taskId";
		...
		if($this->lastError || $numrows != 1) {
			...
			foreach($updatesArr as $field=>$newValue) {
				if(isset($translationArr[$field])) {
					//translate it into the fields that get_tasks() uses (see it's query)
			...
			if($changesCount > 0) {
				//log it!
				$this->logsObj->log_by_class($details, 'update', NULL, $this->recordTypeId, $taskId);
			}
		}
		return($retval);
	...
	//================================================================================================
	/**
	 * Adds a comment to the existing comments for a task.  It's fun.
	 * 
	 * @param $taskId		<int> id of task that will be updated.
	 * @param $comment		<str> comment to add.
	 * 
	 * @return -1			FAIL: comment too short.
	 * @return <special see output of update_task()>
	 */
	function add_comment($taskId, $comment, $subject=NULL) {
		$retval = NULL;
		if(strlen($comment) >1 && is_numeric($taskId)) {
			$taskData = $this->get_task($taskId);
			if(is_array($taskData)) {
				...
				throw new exception("Invalid task (couldn't find record)");
			}
		}
		else {
			throw new exception("Comment too short, or invalid taskId.");
		}
	...
	//================================================================================================
	/** The work to insert a task into the task table.
	 * 
	...
	function create_task($dataArr) {
		...
		$sql = "INSERT INTO task_table $insertStr";
		...
		if($this->lastError || $numrows != 1) {
			...
			$numrows = $this->db->exec("SELECT currval('task_table_task_id_seq')");
			$this->lastError = $this->db->errorMsg();
			...
			if($this->lastError || $numrows != 1) {
				...
				$tmp = $this->db->farray();
				$retval = $tmp[0];
				$this->logsObj->log_by_class("Created new task [task_id=". $retval ."]", 'create');
			}
		}
	...
	//================================================================================================
	private function check_for_auto_updates($taskId, $updatesArr) {
		//get info about the task, so we know what to do...
		$taskData = $this->get_task($taskId);
		
		//set variable for elapsed for later checking.
		$myElapsed = $taskData['estimate_elapsed'];
		if(is_numeric($updatesArr['add_elapsed'])) {
			//update the internal variable (the updates array will be changed later).
			$myElapsed = $this->get_sum_of_elapsed($taskId, $updatesArr['add_elapsed']);
			
			//check if the "estimate_elapsed" field contains more hours than were logged...
			if(($taskData['estimate_elapsed'] + $updatesArr['add_elapsed']) != $myElapsed) {
				//automatically update it.
				$offBy = (($taskData['estimate_elapsed'] + $updatesArr['add_elapsed']) - $myElapsed);
				$this->add_elapsed($taskId, $offBy, $this->logsObj->defaultUid, 'Automatic adjustment');
				$myElapsed = $this->get_sum_of_elapsed($taskId, $updatesArr['add_elapsed']);
			}
			
			$updatesArr['estimate_elapsed'] = $myElapsed;
		}
		elseif(!is_numeric($taskData['estimate_elapsed'])) {
			$myElapsed = 0;
			$updatesArr['estimate_elapsed'] = 0;
		}
		
		//set local variable for current estimate for later checking.
		$myCurrEstimate = $taskData['estimate_current'];
	...
	//================================================================================================
	private function get_comments($taskId) {
		$sql = "SELECT tc.task_comment_id, tc.subject, tc.body, (c.fname || ' ' || c.lname) as creator, " .
			"tc.created, tc.updated FROM task_comment_table AS tc INNER JOIN contact_table AS c ON " .
			"(c.contact_id=tc.creator_contact_id) WHERE task_id=". $taskId ." ORDER BY task_comment_id ASC";
		$numrows = $this->db->exec($sql);
		$this->lastError = $this->db->errorMsg();
		...
		if(strlen($this->lastError) || $numrows < 1) {
			...
			$retval = $this->db->farray_fieldnames('task_comment_id', NULL, 0);
		}
	...
	//=========================================================================
	/** 
	 * Logs the added "elapsed" info for the given task_id.
	 */
	public function add_elapsed($taskId, $addElapsed, $uid, $systemNote=NULL)
	{
		$retval = 0;
		if(is_numeric($addElapsed)) {
			...
			if(strlen($this->lastError) || $numrows !== 1) {
				...
				//now update the task's estimate_elapsed field.
				#$newElapsed = $this->get_sum_of_elapsed($taskId);
			}
		}
	...
	//=========================================================================
	private function get_sum_of_elapsed($taskId, $addThis=NULL) {
		$retval = 0;
		
		$sql = "SELECT sum(add_elapsed) FROM log_estimate_table WHERE task_id=". $taskId;
		$numrows = $this->db->exec($sql);
		$this->lastError = $this->db->errorMsg();
	...
	//=========================================================================
	public function get_hours_logged($taskId, $uid=NULL, $limit=NULL) {
		//retrieve data from log_estimate_table WHERE record_id=$taskId... sort by creation... limit?
		$criteria = array(
			'task_id'	=> $taskId
		);
